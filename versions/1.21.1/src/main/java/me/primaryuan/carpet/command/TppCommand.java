...
/**
 * TppCommand - 早期版本覆盖（1.21.1 ~ 1.21.8）
 *
 * 差异点：
        ...
        new Thread(() -> {
            try {
                ...
                for (int i = 0; i < 50; i++) {
                    ...
                    server.execute(() -> {
                        try {
                            // 早期版本: getPlayer(String) 返回 UUID，需用 stream 查找
                ...
                // 步骤 E: 执行 /player <fakePlayerName> kill（以控制台身份执行）
                server.execute(() -> {
                    server.getCommands().performPrefixedCommand(
                            server.createCommandSourceStack(),
        ...
        new Thread(() -> {
            try {
                ...
                server.execute(() -> {
                    ...
                    player.sendSystemMessage(
                            Component.literal("§a已生成假人: " + fakePlayerName)
                    );
                });
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
