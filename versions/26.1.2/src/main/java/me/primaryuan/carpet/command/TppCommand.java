...
public class TppCommand {
    ...
    private static boolean isAdmin(CommandSourceStack source) {
        ...
        try {
            ...
            if (opsFile.exists() && Files.isReadable(opsFile.toPath())) {
                ...
                return content.contains(player.getUUID().toString()) || content.contains(player.getGameProfile().name());
            }
            return false;
    ...
    private static int setSpawnFakePlayer(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        source.sendSuccess(() -> Component.literal("§a已生成假人: " + fakePlayerName), false);
    ...
    private static int addStation(CommandContext<CommandSourceStack> context) {
        ...
        if (TppConfigManager.stationMap.containsKey(name)) {
            ...
            source.sendSuccess(() -> Component.literal("§e站点 '" + name + "' 已存在（显示名: " + existingDisplay + "）"), false);
            return 0;
        }
    ...
    private static int addStationWithDisplay(CommandContext<CommandSourceStack> context) {
        ...
        source.sendSuccess(() -> Component.literal(
                "§a已添加站点: " + name + " (显示名: " + displayName + ")"), false);
        return 1;
    }
    ...
    private static int removeStation(CommandContext<CommandSourceStack> context) {
        ...
        if (internalName == null) {
            source.sendSuccess(() -> Component.literal("§c站点 '" + input + "' 不存在"), false);
            return 0;
        }
    ...
    private static int setUseCount(CommandContext<CommandSourceStack> context) {
        ...
        if (stationInputFinal != null && !stationInputFinal.isEmpty()) {
            ...
            if (station == null) {
                source.sendFailure(Component.literal("§c站点 '" + stationInputFinal + "' 不存在"));
                return 0;
            }
            ...
            source.sendSuccess(() -> Component.literal(
                    "§a已设置站点 '" + displayName + "' 的右键次数: " + count), false);
        } else {
            // 全局设置
            TppConfigManager.useCount = count;
            TppConfigManager.save();
            source.sendSuccess(() -> Component.literal("§a已设置全局右键次数: " + count), false);
        }
        return 1;
    }
    ...
    private static int showRules(CommandContext<CommandSourceStack> context) {
        ...
        sb.append("§e=== TPP 规则配置 ===\n");
        sb.append("§7全局右键次数: §f").append(TppConfigManager.useCount).append("\n");
        if (!TppConfigManager.stationUseCount.isEmpty()) {
            ...
            for (Map.Entry<String, Integer> entry : TppConfigManager.stationUseCount.entrySet()) {
                ...
                sb.append("\n  §b").append(displayName).append(" §7(§f").append(entry.getKey()).append("§7): §f").append(entry.getValue());
            }
        } else {
            sb.append("§7站点级右键次数: §f(未设置)");
        }
