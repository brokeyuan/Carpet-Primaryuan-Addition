...
public class TppCommand {
    ...
    private static int teleportToStation(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        new Thread(() -> {
            try {
                ...
                // 步骤 C: 逐次执行 use，每次间隔 0.5 秒，确保每次右键独立生效
    ...
    private static int setUseCount(CommandContext<CommandSourceStack> context) {
        ...
        if (stationInputFinal != null && !stationInputFinal.isEmpty()) {
            ...
            if (station == null) {
                source.sendFailure(ServerI18n.tr(source, "carpetprimaryuan.command.tpp.station_not_exists", stationInputFinal));
                return 0;
            }
            TppConfigManager.stationUseCount.put(station, count);
            TppConfigManager.save();
            String displayName = TppConfigManager.getDisplayName(station);
            source.sendSuccess(() -> ServerI18n.tr(source, "carpetprimaryuan.command.tpp.use_count_set_station", displayName, count), false);
        } else {
            // 全局设置
    ...
    private static int showRules(CommandContext<CommandSourceStack> context) {
        ...
        if (!TppConfigManager.stationUseCount.isEmpty()) {
            message = message.append(ServerI18n.tr(source, "carpetprimaryuan.command.tpp.rules_station_counts")).append(Component.literal("\n"));
            for (Map.Entry<String, Integer> entry : TppConfigManager.stationUseCount.entrySet()) {
                ...
                message = message.append(ServerI18n.tr(source, "carpetprimaryuan.command.tpp.rules_station_entry", displayName, entry.getValue())).append(Component.literal("\n"));
            }
        } else {
            message = message.append(ServerI18n.tr(source, "carpetprimaryuan.command.tpp.rules_no_station_counts")).append(Component.literal("\n"));
        }
